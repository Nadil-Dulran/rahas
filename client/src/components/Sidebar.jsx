  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
      <div className='pb-5'>
        <div className='flex justify-between items-center'>
          <img src={assets.logo} alt="logo" className='w-40' />
            <div className="relative py-2" ref={menuRef}>
              <img onClick={() => setMenuOpen(prev => !prev)} src={assets.menu_icon} alt="Menu" className='h-5 cursor-pointer' aria-expanded={menuOpen} />
        ...
        <div className='bg-transparent border-none outline rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
      ...
      <div className='flex flex-col'>
        {filteredUsers.map((user, index) => (
            ...
            key={index}className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id == user._id && 'bg-[#282142]/50'}`}>
              ...
              {unseenMessages[user._id] > 0 && <p className='absolute top-4 right-4 text-xs h-5 w-5 flex justify-center item-center rounded-full bg-violet-500/50'>{unseenMessages[user._id]}</p>}
            </div>
        ))}
      </div>
